...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	/**
	 * 在依赖检查和自动装配时忽略的依赖接口，作为类对象的集合。默认情况下，只有 BeanFactory 接口被忽略。<br/>
	...
	/**
	 * 创建一个新的 AbstractAutowireCapableBeanFactory。<br/>
	...
	/**
	 * 实际创建指定的bean。此时已经进行了预创建处理，例如检查 postProcessBeforeInstantiation 回调。区分默认 bean 实例化、使用工厂方法和自动装配构造函数。<br/>
	 *
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// BeanWrapper 是对 Bean 的包装，其接口中所定义的功能很简单包括设置获取被包装的对象，获取被包装bean的属性描述器
		...
		if (earlySingletonExposure) {
			...
			// 提前将创建的 bean 实例加入到 ObjectFactory 中, 这里是为了后期避免循环依赖
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 如果工厂方法不为空，则使用工厂方法初始化策略,这里推荐看 Factory-Method 实例化 Bean
		if (mbd.getFactoryMethodName() != null) {
			return instantiateUsingFactoryMethod(beanName, mbd, args);
		}

		// Shortcut when re-creating the same bean... 重新创建相同 bean 时的快捷方式
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		...
		try {
			// 调用InitializingBean接口的afterPropertiesSet()和Bean定义中的初始化方法
